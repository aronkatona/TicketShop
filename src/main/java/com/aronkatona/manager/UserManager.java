...
public class UserManager {
	...
	public boolean signUpUser(Map<String,String> reqPar){
		...
		if(existUser(userName) == null){
			...
			saveUser(user);
			String text = "az aktivacios linked: http://localhost:8080/controllerTicketShop/activate." + user.getActivationString();
	...
	public List<Event> getMyEvents(String userName){
		return userService.getUserByName(userName).getEvents();
	}
	
	public List<Chair> getMyChairs(String userName){
		return userService.getUserByName(userName).getChairs();
	}
